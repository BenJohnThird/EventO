...
$Date= date('Y-m-d H:i:s');
 $sql = "UPDATE `events` SET `title`= '".$_POST["title"]."',`place`='".$_POST["place"]."',`proponent`='".$_POST["proponent"]."',`situation`='".$_POST["situation"]."',`notes` = '".$_POST["notes"]."',color = '#FF0000' WHERE id = '".$id."'"; 
...
if(mysqli_num_rows($resultActionPlan) > 0 )
{
	while($row = mysqli_fetch_array($resultActionPlan))
	{
		$sqlHistory = "INSERT INTO `history`(`id`, `Function`, `Time`, `users_ID`, `organization`, `status`)
 		VALUES (null,'Returned ".$row["title"]."','$Date','".$_SESSION["User"]."','$organization',0)";
