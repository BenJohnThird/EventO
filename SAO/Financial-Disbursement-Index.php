...
// THIS WILL VERIFY THE STATUS OF THE FINANCIAL DISBURSMENT
$(document).on('click','#btn_verifyDisbursement', function (){
  var id=$(this).data("id5");
  $.ajax({
    type:'POST',
    url:'Financial-Disbursement-Verify.php',
    data:{id:id},
    dataType:"text",
    success:function(data){
      alert(data);
...
// FOR DISBURSEMENT OF BUDGET DOCUMENTATION
  ...
  $(document).on('click','#btn_add-DisbursementContingency', function (){
   ...
   // FOR DISBURSEMENT ADDING NOTES AND CHANGING STATUS INTO RETURN
  $(document).on('click','#btn_add-FinancialDisbursement', function (){
    ...
    $.ajax({
      ...
      method:"POST",
      data:{id:id,notes:notes,status:status},
      dataType:"text",
      success:function(data){
        alert(data);
        fetchFinancialDisbursement();
