...
include ("Connection.php");
    ...
    $hashed = hash('sha512',$salted);
      ...
      $sqlCheck = "SELECT * FROM student_council where users_ID = '".$_SESSION["User"]."'";
      $resultCheck = mysqli_query($conn, $sqlCheck);
      if(mysqli_num_rows($resultCheck) > 0 )
      {
          while($rowCheck = mysqli_fetch_array($resultCheck))
          {
            $sql = "UPDATE `student_council` SET `Lastname`='$lastname',`Firstname`='$firstname',`Middlename`='$middlename' WHERE users_ID = '".$_SESSION["User"]."'";
             if(mysqli_query($conn, $sql))
             {
    ...
    <div class="col-sm-8 text-left" >
      ...
      <div class = "container">
        <div class = "col-sm-10">
          ...
          <?php
            $sql = "SELECT Username,Lastname,Firstname,Middlename,Email,department_ID,organization_ID,UserLvl FROM `users` inner join student_council on student_council.users_ID = users.ID where ID = '".$_SESSION["User"]."'";
            $result = mysqli_query($conn, $sql);
                if(mysqli_num_rows($result) > 0 )
                {
                    while($row = mysqli_fetch_array($result))
                    {
                     ...
                     <h3><span class="glyphicon glyphicon-user" aria-hidden="true"></span> School Profile</h3>
                       <hr>
                      <form action = "" method = "post">
                       <div class="form-group row">
                         <label for="inputEmail3" class="col-sm-2 col-form-label">Department</label>
                         <div class="col-sm-10">
                           <select name = "department-select" id = "department-select" class = "form-control">';
                           // STORING DEPARTMENT ID IN WORDS
                            $sqlDepInWords = "SELECT * FROM department 
                            where ID = '".$row["department_ID"]."'";
                            $resultInWords = mysqli_query($conn, $sqlDepInWords);
                            if(mysqli_num_rows($resultInWords) > 0 )
                            {
                                while($rowInWords = mysqli_fetch_array($resultInWords))
                                {
                                  echo '<option value = "'.$rowInWords["ID"].'">'.$rowInWords["department_name"].'</option>';
                                }
                            }
                          // SELECT
                           $sqlDepartmentList = "SELECT * FROM department";
                           $resultDepList = mysqli_query($conn,$sqlDepartmentList);
                            if(mysqli_num_rows($resultDepList) > 0 )
                            {
                                while($rowDepList = mysqli_fetch_array($resultDepList))
                                {
                                  echo '<option value = "'.$rowDepList["ID"].'">'.$rowDepList["department_name"].'</option>';
                                }
                            }
                           echo'</select>
                         </div>
                       ...
                       <div class="form-group row">
                         ...
                         <div class="col-sm-10">
                           <select name = "organization-select" id = "organization-select" class = "form-control">';
                          // STORING ORGANIZATION ID IN WORDS
                           $sqlCheckOrganization = "SELECT * FROM users_has_organization where users_ID = '".$row["ID"]."'";
                          ...
                          if(mysqli_num_rows($resultCheckOrganization) > 0 )
                          {
                            ...
                            {
                              // STORING ORGANIZATION ID IN WORDS
                              $sqlOrgInWords = "SELECT * FROM organization 
                              where ID = '".$rowCheckOrganization["organization_ID"]."'";
                                      $resultInWords = mysqli_query($conn, $sqlOrgInWords);
                                      if(mysqli_num_rows($resultInWords) > 0 )
                                      {
                                          while($rowInWords = mysqli_fetch_array($resultInWords))
                                          {
                                            echo '<option value = "'.$rowInWords["ID"].'">'.$rowInWords["organization_name"].'</option>';
                                          }
                                      }
                                }
                          }
                          // SELECT
                           $sqlOrganizationList = "SELECT * FROM organization";
                           $resultOrgList = mysqli_query($conn,$sqlOrganizationList);
                            if(mysqli_num_rows($resultDepList) > 0 )
                            {
                                while($rowOrgList = mysqli_fetch_array($resultOrgList))
                                {
                                  echo '<option value = "'.$rowOrgList["ID"].'">'.$rowOrgList["organization_name"].'</option>';
                                }
                            }
                           echo'</select>
                         </div>
                       ...
                       <div class="form-group row">
                         ...
                         <div class="col-sm-10">
                           <input type="text" readonly name = "position" class="form-control" value = "'.$row["UserLvl"].'" id="inputEmail3" placeholder="Position">
                         </div>
                       </div>
                       <div class="form-group row">
                         <div class="col-sm-10 text-center">
                           <button type="submit" name = "SubmitButton_School" class="btn btn-primary">Update</button>
                         </div>
                       </div>
                     </form>';
                    }
