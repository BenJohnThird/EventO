            ...
            <?php
              ...
              if(mysqli_num_rows($result) > 0 )
              {
                  while($row = mysqli_fetch_array($result))
                  {
                    ...
                    {
                      $sqlCheckCredentials = "SELECT Lastname,Firstname from student_council where users_ID = '".$row["ID"]."'";
                       $resultCheckCredentials = mysqli_query($conn, $sqlCheckCredentials);
                        if(mysqli_num_rows($resultCheckCredentials) > 0 )
                        {
                            ...
                            {
                               echo ''.$_SESSION["Organization"].'-'.$rowCheckCredentials["Lastname"].','.$rowCheckCredentials["Firstname"].'';
                            }
                        }
                    }
                  }
